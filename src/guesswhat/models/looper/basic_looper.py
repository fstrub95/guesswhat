...
class BasicLooper(object):
    ...
    def process(self, sess, iterator, mode, optimizer=list(), store_games=False):
        ...
        for game_data in tqdm(iterator):
            ...
            full_dialogues = [[self.tokenizer.start_token] for _ in range(self.batch_size)]
            answers = [[self.tokenizer.start_token] for _ in range(self.batch_size)]
            ...
            for no_question in range(self.max_no_question):
                ...
                padded_questions, questions, seq_length = \
                    self.qgen.sample_next_question(sess, answers, game_data=game_data, mode=mode)
                ...
                for i in range(self.batch_size):
                    full_dialogues[i] = full_dialogues[i] +  questions[i] + answers[i]


                # Step 1.4 Compute the probability of finding the object after each turn
                if store_games:
                ...
                # Step 1.5 Check if all dialogues are stopped
                has_stop = True
